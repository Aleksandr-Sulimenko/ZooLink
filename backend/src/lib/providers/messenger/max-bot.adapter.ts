  ...
  if (MISSING_TRUST_ANCHOR_CODES.has(err.code)) {
    return new ProviderError(
      ...
      `MAX отказал НЕ по сети, а по ДОВЕРИЮ К СЕРТИФИКАТУ (${err.code}) на хосте ${hostOf(ENDPOINT)}. ` +
        ...
        'издателя и отпечаток предъявленного сертификата. СМЕНА ХОСТА — НЕ ОДНО ДЕЙСТВИЕ, А ДВА ' +
        '(находка №142): сменить адрес в адаптере МАЛО — дверь исходящего периметра отвергнет ' +
        'новый хост ВТОРЫМ отказом, потому что перечень RF_ALLOWED_PROVIDER_HOSTS заморожен и ' +
        'меняется код-ревью, а не правкой .env. Замерено: botapi.max.ru и platform-api.max.ru ' +
        'дверь сегодня НЕ ПУСКАЕТ. И перемерьте доверие: у botapi.max.ru издатель обычный, и ' +
        'русский корень там ЛОМАЕТ доверие. Добавлять корень — ' +
