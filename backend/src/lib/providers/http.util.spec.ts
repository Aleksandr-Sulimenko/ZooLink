...
describe('дверь и редирект (живой loopback)', () => {
  ...
  beforeAll(async () => {
    ...
    a = createServer((req, res) => {
      if (req.url?.includes('429')) {
        // Стенд для №147: вендор просит паузу. Значение параметризовано, чтобы один стенд
        // обслуживал и годный заголовок, и негодные формы.
        const m = /[?&]ra=([^&]*)/.exec(req.url);
        res.statusCode = 429;
        res.setHeader('retry-after', m ? decodeURIComponent(m[1]) : '120');
  ...
  it('🔴 №147: указание вендора «когда вернуться» ДОЖИВАЕТ до вызывающего', async () => {
    // МУТАНТ (красное-до): не читать retry-after — поле пусто, и политика повторов Phase 3 будет
    // УГАДЫВАТЬ паузу; на 429 угадывание и есть способ добить вендора и получить бан ключа.
    const err = (await fetchJson('проба', `${aUrl}/429`).catch((e: unknown) => e)) as ProviderError;
  ...
  it.each([
    ['мусор', 'Wed, 21 Oct 2026 07:28:00 GMT'], // HTTP-date НЕ разбираем — честное «не знаю»
    ['отрицательное', '-5'],
    // ЗНАЧЕНИЕ ЗАГОЛОВКА — ТОЛЬКО ASCII: Node отвергает кириллицу в значении HTTP-заголовка, и
    // стенд падал ДО проверки, то есть ось краснела не на предмете. Шестой за сутки случай закона
    // «кириллица не переживает шов» — и снова поймал прогон, а не вычитка.
    ['не число', 'soon'],
    ['за потолком суток', '999999999'],
    // 🔴 ЭТИ ТРИ ДОКАЗЫВАЮТ, ЧТО ПРОВЕРКА «ТОЛЬКО ЦИФРЫ» НЕСУЩАЯ. Мутант «снять её» оставался
    // ЗЕЛЁНЫМ на четырёх входах выше: их всех отсекает вторая проверка (NaN не safe-integer).
    // Зелёный мутант — знак неполноты ОСИ, а не лишнего кода. Без регэкспа Number() принял бы:
    // шестнадцатеричное «0x10» как 16 с, экспоненту «1e3» как 1000 с, а ПУСТУЮ строку — как 0,
    // то есть «возвращайся немедленно»: чужой заголовок управлял бы нашей политикой повторов.
    ['шестнадцатеричное', '0x10'],
    ['экспонента', '1e3'],
    ['пустое значение', ''],
  ])('🔴 №147: негодный Retry-After (%s) даёт «НЕ ЗНАЮ», а не ноль', async (_имя, значение) => {
    // Несущее: ноль политика прочла бы как «возвращайся немедленно» — то есть чужой заголовок
    // управлял бы нашим поведением. Отсутствие знания обязано выглядеть как отсутствие знания.
    const err = (await fetchJson('проба', `${aUrl}/429?ra=${encodeURIComponent(значение)}`).catch(
      (e: unknown) => e,
    )) as ProviderError;
  ...
  it('🔴 №135: в fetch уходит УЖЕ РАЗОБРАННЫЙ адрес — второго разбора той же строки нет', async () => {
    // МУТАНТ (красное-до): вернуть `fetch(url, …)` — ось краснеет. Цена второго разбора линейна по
    // длине адреса (замер находки: 954 нс на коротком, 1,4 мс на 65536 символах), а у геокодера
    // длина управляется ИЗВНЕ — пользовательская строка уходит в адресную строку без ограничения.
    // Ось меряет СВОЙСТВО (что именно передано), а не время: время шумит на общей машине и
    // стерегло бы стенд, а не код.
    const real = globalThis.fetch;
    let вход: unknown = null;
    globalThis.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
      вход = input;
      return real(input, init);
    };
    try {
      await fetchJson('проба', `${aUrl}/302`).catch(() => undefined);
    } finally {
      globalThis.fetch = real;
    }
    expect(вход).toBeInstanceOf(URL);
  });

  it('🔴 №139: ОБА пути чтения тела дают ОДНО И ТО ЖЕ — и короткий, и склейка', async () => {
    // Лечение цены (общий декодер + отказ от лишней копии на ОДНОМ куске) касается КОРРЕКТНОСТИ,
    // поэтому ось меряет её, а не время: время шумит на общей машине и стерегло бы стенд.
    // Тело заведомо БОЛЬШЕ куска undici (64 КиБ) — значит идёт путь склейки; и многобайтовые
    // символы на границе кусков ловят классическую ошибку «декодировать каждый кусок отдельно».
    const { createServer } = await import('node:http');
    const текст = 'п'.repeat(100_000); // кириллица = 2 байта на символ, граница кусков внутри неё
    const srv = createServer((_req, res) => {
      res.setHeader('content-type', 'application/json');
      res.end(JSON.stringify({ v: текст }));
    });
    const port = await new Promise<number>((r) =>
      srv.listen(0, '127.0.0.1', () => r((srv.address() as { port: number }).port)),
    );
    try {
      const got = await fetchJson<{ v: string }>('проба', `http://127.0.0.1:${port}/big`);
      expect(got.v).toHaveLength(текст.length);
      expect(got.v).toBe(текст); // ни одного потерянного или порченого символа на стыке кусков
    } finally {
