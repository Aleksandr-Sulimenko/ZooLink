 ...
 *
 * ПОЧЕМУ ЗДЕСЬ, А НЕ В КАЖДОМ АДАПТЕРЕ. Здесь единственный `fetch` во всём `backend/src` — одна
 * точка на ЗАПРОСЫ ПРОВАЙДЕРОВ, и потому их периметр виден целиком.
 * 🔶 УТВЕРЖДЕНИЕ СУЖЕНО ДО ЗАМЕРЕННОГО (находка №122). Здесь стояло «иных исходящих клиентов нет»,
 * и это было ЛОЖНО уже в момент написания: `lib/observability/sentry.ts` импортирует `@sentry/node`,
 * чей транспорт зовёт `https.request` мимо этой двери. Верно про `fetch` — неверно про «клиентов».
 * ВТОРЫЕ ДВЕРИ НЕ ЗАМАЛЧИВАЮТСЯ И НЕ ПРОЩАЮТСЯ МОЛЧА: они объявлены поимённо в реестре
 * `SECOND_DOORS` (`scripts/check-rf-residency.sh`, ось 6b) — с модулем и с указанием, ЧЕМ
 * проверяется их хост, — и реестр ДВУСТОРОННИЙ: запись, чей файл или модуль исчез, красит гейт.
 * Значит «сколько у нас дверей» отвечает ПРИБОР, а не этот комментарий.
 *
