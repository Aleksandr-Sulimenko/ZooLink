...
export async function fetchJson<T>(
  ...
  try {
    ...
    // 🔶 `manual` ВМЕСТО `error` (находка №144, 31.08.2026) — ОТКАЗ ТОТ ЖЕ, ДИАГНОЗ ЧЕСТНЕЕ.
    // При `error` undici бросает «fetch failed / unexpected redirect», и заголовок `Location` до
    // нас НЕ ДОХОДИТ ВООБЩЕ — а текст отказа при этом утверждал про второй хоп, что дверь его «не
    // видела», то есть утверждал про то, чего код не смотрел. В самом вероятном случае (вендор
    // завёл обычный 301 apex→www) это утверждение ЛОЖНО, а настоящее лечение — сменить ENDPOINT —
    // в тексте не называлось вовсе, и оператор шёл искать утечку, которой нет.
    // ФОРМА ОТКАЗА НЕ ОСЛАБЛЕНА: `manual` НЕ идёт по редиректу — он возвращает сам ответ 3xx, а мы
    // отвергаем его ЯВНО ниже. Даже если эту ветку однажды снимут, 3xx не пройдёт: `res.ok` ложно
  ...
  // ═══ ПЕРЕНАПРАВЛЕНИЕ: ОТКАЗ, КОТОРЫЙ НАЗЫВАЕТ КУДА ВЕЛО И ЧТО ДЕЛАТЬ (находка №144) ═══
  // Тело отменяем, как и в соседней ветке: непрочитанное держит сокет бессрочно (замер круга 3).
  if (res.status >= 300 && res.status < 400) {
    const location = res.headers.get('location');
    await res.body?.cancel().catch(() => undefined);
    let куда = 'заголовка Location в ответе НЕТ — куда вело, площадка не сказала';
    if (location !== null) {
      ...
      куда =
        host === null
          ? 'Location не разбирается как адрес, поэтому цель назвать нельзя'
          : isAllowedProviderHost(host)
            ? `вело на «${host}» — ЭТОТ ХОСТ ДВЕРЬ ЗНАЕТ. Похоже на переезд вендора внутри его же ` +
              'периметра (apex→www, смена пути): лечение — поправить адрес в АДАПТЕРЕ, а не искать утечку'
            : `вело на «${host}» — этого хоста в перечне НЕТ. Новый хост входит код-ревью, как новый ` +
              'провайдер; правкой .env перечень не меняется (он заморожен по построению)';
    }
    throw new ProviderError(
      provider,
      'config',
      `площадка ответила ПЕРЕНАПРАВЛЕНИЕМ (HTTP ${res.status}), а дверь его не проходит: проверка ` +
        'хоста стережёт только ПЕРВЫЙ хоп, и 307/308 унесли бы ТЕЛО с ключом на второй. ' +
        `КУДА ВЕЛО — СМОТРЕЛИ: ${куда}. Это ПОСТОЯННЫЙ отказ: повтор не поможет.`,
      undefined,
      undefined,
      true, // mayHaveArrived: 3xx есть ОТВЕТ площадки — запрос она приняла и обработала
    );
