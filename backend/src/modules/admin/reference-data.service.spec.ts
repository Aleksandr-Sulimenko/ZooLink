...
import { ReferenceDataService, resolveLang } from './reference-data.service';
...
const speciesRow = {
  id: 1,
  code: 'dog',
  name_localized: { ru: 'Собака', en: 'Dog' },
  sort_order: 0,
  market: 'pet',
  is_active: true,
...
const breedRow = {
  ...
  species_id: 1,
  code: 'akita',
  name_localized: { ru: 'Акита', en: 'Akita' },
  sort_order: 0,
  is_active: true,
  created_at: new Date('2026-06-22T00:00:00Z'),
  updated_at: new Date('2026-06-22T00:00:00Z'),
...
const cityRow = {
  id: 9,
  name_localized: { ru: 'Москва', en: 'Moscow' },
  sort_order: 0,
  is_active: true,
  created_at: new Date('2026-06-22T00:00:00Z'),
  updated_at: new Date('2026-06-22T00:00:00Z'),
...
describe('resolveLang', () => {
  it('parses ru/en, falls back to ru for unsupported or missing', () => {
...
describe('ReferenceDataService.list', () => {
  ...
  it('orders by sort_order then id', async () => {
    const { svc, species } = setup();
    await svc.list('species', { includeInactive: false, page: 1, limit: 20, skip: 0 }, undefined);
    expect(species.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ orderBy: [{ sort_order: 'asc' }, { id: 'asc' }] }),
    );
  });

  it('public read resolves the name string for Accept-Language (no nameLocalized)', async () => {
    const { svc } = setup();
    const res = await svc.list('species', { includeInactive: false, page: 1, limit: 20, skip: 0 }, undefined, 'en');
    expect(res.items[0].name).toBe('Dog');
    expect(res.items[0].nameLocalized).toBeNull();
  });

  it('admin read returns both locales (nameLocalized), no resolved name', async () => {
    const { svc } = setup();
    const res = await svc.list('species', { includeInactive: false, page: 1, limit: 20, skip: 0 }, admin, 'en');
    expect(res.items[0].nameLocalized).toEqual({ ru: 'Собака', en: 'Dog' });
    expect(res.items[0].name).toBeNull();
  });

  it('searches the JSONB name paths + code for datasets that have a code column', async () => {
    const { svc, species } = setup();
    await svc.list('species', { includeInactive: false, page: 1, limit: 20, skip: 0, search: 'do' }, undefined);
    const call = species.findMany.mock.calls[0][0] as { where: { OR: unknown[] } };
    expect(call.where.OR).toHaveLength(3); // name_localized ru + en + code
  });

  it('searches name paths only (no code) for cities', async () => {
...
describe('ReferenceDataService.getById', () => {
  it('404s when missing', async () => {
    ...
    await expect(svc.getById('species', 999, undefined)).rejects.toBeInstanceOf(NotFoundException);
  });

  it('returns the entry with a weak ETag derived from updated_at', async () => {
    const { svc } = setup();
    const { entry, etag } = await svc.getById('species', 1, undefined, 'ru');
    expect(entry.id).toBe(1);
    expect(entry.name).toBe('Собака');
    expect(etag).toBe(weakEtag('species:1', speciesRow.updated_at));
  });
});

describe('ReferenceDataService.create', () => {
  it('creates a species, stamps provenance, and audits with the acting principal', async () => {
    const { svc, species, record } = setup();
    await svc.create('species', { code: 'cat', nameLocalized: { ru: 'Кошка', en: 'Cat' }, market: 'pet' }, admin);
    expect(species.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          ...
          is_active: true,
          sort_order: 0,
          name_localized: { ru: 'Кошка', en: 'Cat' },
          created_by: 'admin-1',
          updated_by: 'admin-1',
        }),
      }),
    ...
    expect(record).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        actorId: 'admin-1',
        actorRole: 'ADMIN',
        entityIdInt: expect.any(Number),
      }),
    );
  });

  it('rejects a nameLocalized with no non-empty locale', async () => {
    const { svc } = setup();
    await expect(
      svc.create('cities', { nameLocalized: { ru: '', en: '' } }, admin),
  ...
  it('rejects a code on cities (field not applicable)', async () => {
    const { svc } = setup();
    await expect(
      svc.create('cities', { code: 'msk', nameLocalized: { ru: 'Москва', en: 'Moscow' } }, admin),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('requires speciesId for breeds', async () => {
    const { svc } = setup();
    await expect(
      svc.create('breeds', { code: 'akita', nameLocalized: { ru: 'Акита', en: 'Akita' } }, admin),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('rejects a breed whose speciesId does not exist (referential integrity)', async () => {
    ...
    await expect(
      svc.create('breeds', { code: 'akita', speciesId: 42, nameLocalized: { ru: 'Акита', en: 'Akita' } }, admin),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('maps a unique-violation to 409 Conflict', async () => {
    ...
    await expect(
      svc.create('species', { code: 'dog', nameLocalized: { ru: 'Собака', en: 'Dog' } }, admin),
  ...
  it('rejects market on a breed (not applicable)', async () => {
    const { svc } = setup();
    await expect(
      svc.create('breeds', { code: 'x', speciesId: 1, nameLocalized: { ru: 'X', en: 'X' }, market: 'pet' } as never, admin),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
});
...
describe('ReferenceDataService.update', () => {
  it('428s when If-Match is missing', async () => {
    const { svc } = setup();
    await expect(
      svc.update('species', 1, { nameLocalized: { ru: 'Пёс', en: 'Doggo' } }, undefined, admin),
  ...
  it('412s when If-Match is stale', async () => {
    const { svc } = setup();
    await expect(
      svc.update('species', 1, { nameLocalized: { ru: 'Пёс', en: 'Doggo' } }, 'W/"stale"', admin),
    ).rejects.toMatchObject({ status: 412 });
  });

  it('updates with a valid If-Match, stamps updated_by, and audits before/after', async () => {
    const { svc, species, record } = setup();
    const etag = weakEtag('species:1', speciesRow.updated_at);
    const { entry } = await svc.update('species', 1, { nameLocalized: { ru: 'Пёс', en: 'Doggo' } }, etag, admin);
    expect(entry.nameLocalized).toEqual({ ru: 'Пёс', en: 'Doggo' });
    expect(species.update).toHaveBeenCalledWith(
      expect.objectContaining({ data: { name_localized: { ru: 'Пёс', en: 'Doggo' }, updated_by: 'admin-1' } }),
    );
    expect(record).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'reference_data.updated', entityIdInt: 1 }),
    );
  });
...
describe('ReferenceDataService.toggleActive', () => {
  it('flips is_active and audits a deactivation', async () => {
    ...
    expect(species.update).toHaveBeenCalledWith(
      expect.objectContaining({ data: { is_active: false, updated_by: 'admin-1' } }),
    );
    expect(record).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'reference_data.deactivated', entityIdInt: 1 }),
    );
  });
...
describe('ReferenceDataService.form', () => {
  it('includes code+market for species, code+speciesId for breeds, neither for cities', () => {
    const { svc } = setup();
    expect(Object.keys(svc.form('species').fields)).toEqual(['code', 'nameLocalized', 'market', 'sortOrder', 'isActive']);
    expect(Object.keys(svc.form('breeds').fields)).toEqual(['code', 'speciesId', 'nameLocalized', 'sortOrder', 'isActive']);
    expect(Object.keys(svc.form('cities').fields)).toEqual(['nameLocalized', 'sortOrder', 'isActive']);
  });
});
