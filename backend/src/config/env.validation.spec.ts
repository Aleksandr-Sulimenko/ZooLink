...
describe('.env.example — контракт окружения стартует ДОСЛОВНО (№148/№157/№172)', () => {
  ...
  // ═══ ПОСЫЛКА ОСИ ИСПРАВЛЕНА ЗАМЕРОМ (31.08.2026, лечение №174) ═══
  // Ось звучала «шаблон принимается validateEnv» и гнала его КАК ЕСТЬ — а шаблон объявляет
  // `NODE_ENV=production` (строка 5). Значит «шаблон обязан стартовать дословно» и «сырая копия
  // обязана быть ОТВЕРГНУТА в проде» (негатив-контроль gen-env.spec) требовали
  // ПРОТИВОПОЛОЖНОГО ОТ ОДНОГО И ТОГО ЖЕ ВХОДА. Пока METRICS_TOKEN был пуст, побеждала вторая;
  // 29.08 заглушку заполнили — победила первая, и негатив-контроли молча покраснели.
  // ЧТО ИЗМЕРЯЕТСЯ ТЕПЕРЬ, ОБОИМИ ПОЛЮСАМИ:
  //  · ФОРМА контракта цела — шаблон стартует как ФОРМА, то есть в dev (ради этого №148 и заводили:
  //    контракт, который не стартует вовсе, не учит ничему);
  //  · и он ОТВЕРГАЕТСЯ в production — потому что это опись ключей, а не боевая конфигурация.
  // Байты шаблона не тронуты. Вопрос «должен ли сам шаблон нести NODE_ENV=production» — отдельный,
  // назван держателю, а не решён здесь молча.
  it('ШАБЛОН СТАРТУЕТ ДОСЛОВНО, БЕЗ ЕДИНОЙ ПРАВКИ — иначе контракт не учит ничему (№148)', () => {
    // ДОСЛОВНО значит ДОСЛОВНО: с 31.08.2026 шаблон сам несёт `NODE_ENV=development` (решение
    // держателя — опись не является боевой конфигурацией), поэтому подменять здесь нечего.
    // Прежняя редакция этой оси подставляла режим снаружи — это была ЗАПЛАТА на ложной посылке
    // шаблона, а не измерение. Заплата снята вместе с посылкой.
    expect(изШаблона().NODE_ENV).toBe('development');
    expect(() => validateEnv(изШаблона())).not.toThrow();
  });

  it('🔴 И ОТВЕРГАЕТСЯ В PRODUCTION — прод-природа собирается ВНЕШНЕ, форма её не несёт (№174)', () => {
    // МУТАНТ (красное-до): снять отказ по имени в validateEnv — ось краснеет, и вместе с ней
    // возвращается дыра: сырая копия шаблона поднимает прод с ПУБЛИЧНЫМ токеном метрик.
    let сообщение = '';
    try {
      validateEnv({ ...изШаблона(), NODE_ENV: 'production' });
    } catch (e: unknown) {
      сообщение = e instanceof Error ? e.message : String(e);
    }
    expect(сообщение).toContain('METRICS_TOKEN');
    expect(сообщение).toContain('ЗАГЛУШКА ШАБЛОНА');
  });
