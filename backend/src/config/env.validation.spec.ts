...
describe('.env.example — контракт окружения стартует ДОСЛОВНО (№148/№157/№172)', () => {
  ...
  it('ШАБЛОН СТАРТУЕТ ДОСЛОВНО КАК ФОРМА (dev) — иначе контракт не учит ничему (№148)', () => {
    expect(() => validateEnv({ ...изШаблона(), NODE_ENV: 'development' })).not.toThrow();
  });

  it('🔴 И ОТВЕРГАЕТСЯ В PRODUCTION — заполненная заглушка опаснее пустого значения (№174)', () => {
