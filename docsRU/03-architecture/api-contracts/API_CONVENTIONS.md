...
- **Идемпотентность (§11):** `IDEMPOTENCY_KEY_REUSED` (422, тот же ключ с другим телом запроса),
  `IDEMPOTENCY_KEY_IN_PROGRESS` (409, запрос с тем же ключом ещё выполняется — повторить после подсказки
  `Retry-After`).
...
Ведра — **на клиента**, по адресу клиента, который край передаёт в `X-Real-IP`, и никогда не по сырому адресу
сокета (за обратным прокси это одно и то же значение для всех вызывающих). Как заголовок делается достоверным
и как группируется IPv6 — в `nfr/security.md` → «Безопасность API». Пробы здоровья и scrape метрик исключены и
не несут заголовков `X-RateLimit-*`.
...
> **ETag объявления (ADR-0035):** ETag ресурса listing выводится из выделенной колонки версии контента
> (`listings.content_updated_at`), **а не** `updated_at`. Его двигают только записи, меняющие видимое
> клиенту состояние объявления, поэтому записи read-path/системные (инкремент `view_count` на публичном
> `GET /listings/{id}`, `escalated_at`, пересчёт производного `market`) **не** ротируют ETag — публичное
> чтение никогда не инвалидирует удерживаемый продавцом/оператором валидатор (нет ложного `412`
> edit-lockout). Остальные ресурсы остаются на `updated_at`, если у них нет того же риска записи из
> read-path.
...
ключом → сохранённый ответ (включая значимые заголовки ответа, минимум `ETag`); тот же ключ с другим телом → `422`
(`IDEMPOTENCY_KEY_REUSED`). Ключ захватывается **атомарно в начале** первого запроса краткоживущей in-progress-резервацией:
**параллельный** запрос с тем же ключом, пришедший пока первый ещё выполняется, получает `409`
(`IDEMPOTENCY_KEY_IN_PROGRESS`) + подсказку `Retry-After`, а не выполняет дублирующий побочный эффект параллельно;
если первый запрос упал, резервация снимается, и повтор может пройти (для реплея кэшируются только успешные ответы).
Это HTTP-дополнение к UNIQUE-ограничениям БД (`favorites`, OPEN-дедуп `content_reports`, `payment_transactions.idempotency_key`).

> **(round-N, normative — in-flight идемпотентность, аудит 2026-06-30) WHAT:** §11 теперь требует атомарную
> in-progress-резервацию в начале запроса (а не только сохранение по завершении), `409 IDEMPOTENCY_KEY_IN_PROGRESS`
> для параллельного дубликата, реплей значимых заголовков (`ETag`) и снятие резервации при ошибке.
> **WHY:** прежнее «сохранение по завершении» оставляло окно, в котором два идентичных POST (двойной тап / повтор
> клиента) могли выполниться оба до того, как любой сохранил результат — именно тот backstop, на который обопрутся
> новые потоки offering/booking (Part B), где дубликат не ловит UNIQUE-ограничение БД.
> **WHY-BETTER-for-the-whole-project:** закрывает шов replay/race существующим примитивом платформы (`SET NX` в Redis)
> — без новой зависимости; идемпотентность остаётся HTTP-дополнением к UNIQUE-ключам БД; реплей заголовков сохраняет
> работу `ETag`/`If-Match` (оптимистичный контроль версий) на реплее ответа. EN-канон обновлён.
