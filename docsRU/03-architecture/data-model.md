...
- Роли и права доступа (`users.role` — единственная авторитетная/основная роль)
- `user_roles` - связующая таблица мультиролей (ADR-0022, миграция 0034), **спящая** в MVP: резервирует
  шов «один аккаунт — много ролей», пока `users.role` остаётся единственным источником авторизации (см. ниже)
...
12. `user_roles.user_id → users.id` (ON DELETE CASCADE) и `user_roles.actor_id → users.id` (ON DELETE SET NULL) — связующая таблица мультиролей (ADR-0022)
...
| `notification_logs` | `specs/13-notification-domain.md` | Журнал доставки (SENT/DELIVERED/FAILED/BOUNCED). `type` — **канал** доставки (`EMAIL`/`SMS`/`IN_APP` — `IN_APP` добавлен миграцией 0030, ADR-0021); `notification_templates.type` — **источник** контента (`EMAIL`/`SMS`). Воркерный `NotificationConsumer` (первый реальный outbox-consumer) пишет по одной `IN_APP`-строке на транзакционное событие/получателя, идемпотентно по `idempotency_key = event.id‖recipient‖template`. |
| `ownership_transfers` | `specs/statemachines/ownership_transfer_state_machine.md` · [`../04-decisions/0013-mvp-ownership-transfer.md`](../04-decisions/0013-mvp-ownership-transfer.md) | Процессная сущность стейт-машины передачи (в отличие от `animal_ownership_history` — журнала свершившихся фактов). **Передача владения ВХОДИТ в MVP** как упрощённый прямой флоу (ADR-0013): `PENDING → COMPLETED` при принятии получателем, `PENDING → CANCELLED` при отклонении/отмене/истечении. Триггер owner-lock `trg_animals_immutable_and_owner` разрешает изменение `owner_id`/`organization_id` **только** через контролируемый путь передачи — сервис передачи выставляет транзакционно-локальный GUC `app.ownership_transfer='on'` (`set_config('app.ownership_transfer','on', true)`) в той же транзакции; триггер разрешает изменение iff `current_setting('app.ownership_transfer', true)='on'`, иначе raise. Первый кастомный GUC `app.*` в кодовой базе. Тяжёлый набор верификации `IN_PROGRESS`/`FAILED` (платёж/вет/юр.) остаётся forward-compatible **формой** на существующих колонках, за `feature_toggles.ownership_transfer_verification` (по умолчанию off). **Миграция 0023 приземлена (ADR-0013 §3): в CHECK статуса добавлен `CANCELLED`; `from/to_organization_id` (+ CHECK ровно-одно-из user/org с каждой стороны); `completed_at`; `initiated_by/responded_by_principal_type` (HUMAN\|AGENT, ADR-0011); `initiated_by_user_id`/`responded_by_user_id` (действующий пользователь каждого акта — `from_user_id` — это from-сторона и NULL для org-from, поэтому не может служить актором-инициатором); `transfer_reason`; частичный уникальный индекс `UNIQUE (animal_id) WHERE status='PENDING'` (единственный активный PENDING, INV-4). Дублирующее определение `trg_animals_immutable_and_owner` сведено к одному каноническому GUC-aware телу. `animal_ownership_history` (OQ-1 решён = вариант (a)): `owner_id` сделан nullable + добавлен `organization_id` + CHECK ровно-одно-из, чтобы интервалы org-владения были записываемы.** |
...
| `confirmed_sales` | `specs/18-reputation.md` · [`../04-decisions/0038-confirmed-sale-record-of-truth.md`](../04-decisions/0038-confirmed-sale-record-of-truth.md) | **Append-only запись-источник-истины о том, что реальная сделка состоялась** (ADR-0038, миграция 0039) — корень proof-of-transaction, на котором висит репутация (ADR-0039); самостоятельная сущность, НЕ проекция (несёт новые факты, которых нет в источниках: встречное подтверждение покупателя, спор, снапшот актора). Привязана к COMPLETED `ownership_transfers` (авто-CONFIRMED, пишется **в той же tx**, что и accept передачи, чтобы сильнейший сигнал сделки не терялся) или к листингу `markSold` (встречное подтверждение покупателя, отложено за `feature_toggles.sale_buyer_confirmation`). Неизменяемость через **переиспользованный** `trg_confirmed_sales_immutable` (НЕ изобретать второй путь неизменяемости). Полиморфный субъект `offering_type`/`offering_id` (диалект миграции 0032, сейчас только `ANIMAL_LISTING`, widen-additively) + конкретные `listing_id`/`animal_id`. Кэш производного `market` (ADR-0018/0033, жёсткий сплит ADR-0002). `anchor_type` ∈ {`TRANSFER`,`LISTING_MARK_SOLD`}. **ТОЛЬКО-CONFIRMED ФАКТ (ADR-0038 §4 Дополнение, миграция 0041):** строка СУЩЕСТВУЕТ тогда и только тогда, когда сделка — подтверждённый факт — `status` держит **суженный `CHECK (status = 'CONFIRMED')`** (Q2-надгробие; прежняя 5-значная машина сужена, не удалена) и без `DEFAULT`; PENDING/переговорный жизненный цикл переехал в `sale_confirmations`. Переговорные колонки (`nominated_buyer_user_id`/`seller_confirmed_at`/`buyer_confirmed_at`/`expires_at`) и `idx_confirmed_sales_confirm_scan` удалены. `UNIQUE (ownership_transfer_id)` остаётся **ПОЛНЫМ** (Q1 — подтверждённая сделка-передача постоянна и никогда не заменяется; отмена/спор после подтверждения — новая запись, ADR-0040) = одна сделка на передачу (зеркало INV-4). Actor-снапшот `actor_id` + `actor_principal_type` (HUMAN\|AGENT). `amount_minor BIGINT` **зарезервирована nullable, off-record по умолчанию (владелец 2026-07-09)** — НИ один путь кода её не пишет. Party-FK `ON DELETE SET NULL` (ФЗ-152 псевдонимизация). **СПЯЩИЙ захват в MVP** — потребителя нет, эндпоинт её не читает; путь передачи эмитит `ConfirmedSale.Confirmed` in-tx (НЕ `.Created` — строка-передача рождается CONFIRMED без фазы PENDING). |
| `sale_confirmations` | `specs/18-reputation.md` · [`../04-decisions/0038-confirmed-sale-record-of-truth.md`](../04-decisions/0038-confirmed-sale-record-of-truth.md) | **ИЗМЕНЯЕМЫЙ жизненный цикл встречного подтверждения покупателя для markSold** (ADR-0038 §4 Дополнение, миграция 0041) — «изменяемое состояние»-спутник append-only ФАКТА `confirmed_sales` (проектный паттерн **неизменяемый факт + изменяемое состояние**, симметрично с `reviews`/`review_states`). Держит `status` ∈ {`PENDING_CONFIRMATION`,`CONFIRMED`,`DISPUTED`,`EXPIRED`,`CANCELLED`} (изменяемый), `seller_confirmed_at`/`buyer_confirmed_at`/`expires_at`, `nominated_buyer_user_id`, полиморфный субъект + стороны (та же форма, что у `confirmed_sales`) и обратный указатель `confirmed_sale_id`, ставящийся при CONFIRM. **`chk_sale_conf_confirmed_link`** биусловие (зеркало `chk_moddec_override`): `status='CONFIRMED' ⇔ confirmed_sale_id IS NOT NULL` — CONFIRMED-переговоры породили ровно одну строку-факт. **`uq_sale_conf_live_per_listing`** частичный уникальный `(listing_id) WHERE status IN ('PENDING_CONFIRMATION','DISPUTED')` = одни ЖИВЫЕ переговоры на листинг (отменённые/истёкшие освобождают листинг — спека §4). Триггер `updated_at` (авто-навешен). Путь **ПЕРЕДАЧИ сюда никогда не пишет** (завершённая передача рождается CONFIRMED напрямую в `confirmed_sales`). **СПЯЩИЙ** за `feature_toggles.sale_buyer_confirmation`. |
| `reviews` | `specs/18-reputation.md` · [`../04-decisions/0039-reputation-storage-model.md`](../04-decisions/0039-reputation-storage-model.md) | **Append-only, один-ТЕКУЩИЙ-на-(сделку, направление) отзыв с доказательством сделки** (ADR-0039, миграции 0040/0041) — `rating` 1..5 + опциональный `body`, написанный стороной **CONFIRMED** `confirmed_sales` о другой (FK `confirmed_sale_id` ON DELETE CASCADE; нет подтверждённой сделки ⇒ нет отзыва). **Правка-в-grace = НОВАЯ замещающая строка, называющая своего предшественника через ОБРАТНЫЙ указатель `supersedes_review_id`** (ADR-0039 §3 Дополнение, миграция 0041 — модель `moderation_decisions`; append-only-safe, т.к. новая строка пишет о себе, а не о старой строке; `ON DELETE RESTRICT`). Прямой `superseded_by_id` заменён. **«Текущий» разрешается через VIEW `reviews_current`** (голова цепочки = строка, которую ничто не supersedes); `seq BIGINT GENERATED ALWAYS AS IDENTITY` — зарезервированный детерминированный tie-break (урок миг. 0036), НИКОГДА не `created_at`. **DB-инварианты:** `uq_reviews_supersedes (supersedes_review_id) WHERE NOT NULL` = ≤1 преемник на предшественника → **ЛИНЕЙНАЯ** цепочка правок (Q5 — НАМЕРЕННО строже `moderation_decisions`, который может ветвиться; НЕ «унифицировать»); `uq_reviews_root_per_direction (confirmed_sale_id, direction) WHERE supersedes_review_id IS NULL` = одна корневая цепочка → одна голова → **«один текущий на (сделку, направление)» остаётся DB-инвариантом** (Q6). `direction` ∈ {`BUYER_ON_SELLER`,`SELLER_ON_BUYER`}. **Изменяемое состояние модерации + double-blind видимость вынесены в `review_states`** (§3 Дополнение β). Фасет-колонки (`facet_description_accuracy`/`facet_communication`/`facet_as_described`) зарезервированы **СПЯЩИМИ** (форк 5). Party-FK (`reviewer_user_id`/`subject_user_id`) `ON DELETE SET NULL` = ФЗ-152 псевдонимизация-автора-с-сохранением-рейтинга (форк 8, под юр. одобрением). Actor-снапшот `actor_id` + `actor_principal_type` (HUMAN\|AGENT — авторство ВЫКЛ, ADR-0040 §3). Неизменяемость через **переиспользованный** `trg_reviews_immutable`. **СПЯЩАЯ в MVP** — эндпоинтов нет, поведение под `feature_toggles.reputation_reviews`. |
| `review_states` | `specs/18-reputation.md` · [`../04-decisions/0039-reputation-storage-model.md`](../04-decisions/0039-reputation-storage-model.md) | **ИЗМЕНЯЕМОЕ состояние модерации + видимости на отзыв** (ADR-0039 §3 Дополнение β, миграция 0041) — «изменяемое состояние»-спутник append-only ФАКТА `reviews` (проектный паттерн **неизменяемый факт + изменяемое состояние**, симметрично с `sale_confirmations`). PK `review_id` FK `reviews` `ON DELETE CASCADE` (состояние висит на стабильном id отзыва; чтения join-ят). Держит `moderation_status` ∈ {`PENDING`,`APPROVED`,`REJECTED`,`CHANGES_REQUESTED`} (`chk_review_states_mod_status`) и `is_visible` (гейт double-blind release, DEFAULT FALSE, форк 2) — **оба мутируют по замыслу** (переходы модерации; double-blind release планировщиком), поэтому они не могут жить на append-only `reviews`. `moderated_at` + actor-снапшот `actor_id`/`actor_principal_type` (кто подвинул состояние — модератор/агент). Триггер `updated_at` (авто-навешен). **СПЯЩАЯ** за `feature_toggles.reputation_reviews`. |
| `reputation_aggregates` | `specs/18-reputation.md` · [`../04-decisions/0039-reputation-storage-model.md`](../04-decisions/0039-reputation-storage-model.md) | **Производный кэш рейтинга на-(субъект, рынок)** (ADR-0039 §1/§2, миграция 0040) — `review_count`/`rating_sum`/`rating_avg` + гистограмма звёзд `dist_1..dist_5`, пересчитывается инкрементально (O(Δ)) на пути событий отзывов, читается одним индексированным PK-запросом (< 50 мс). **PK `(subject_user_id, market)` = граница ADR-0002 структурно** (отзыв о livestock никогда не двигает pet-агрегат; кросс-рыночное чтение требует явного `?market=`). **`rating_avg` — колонка Postgres `GENERATED ALWAYS AS ROUND(rating_sum::numeric / NULLIF(review_count,0), 2) STORED`** — только-пересчёт, **никогда не пишется вручную** (прямая запись отвергается Postgres): покупаемый/подделываемый рейтинг доверия структурно невозможен (форк 7 / TP-8); NULL при `review_count = 0`. **НЕ append-only** (изменяемый пересчитываемый кэш — путь пересчёта UPDATE-ит count/sum, а сгенерированный avg следует). Хук взвешивания по автору (ADR-0040 §1) аддитивен (сейчас не колонка). **СПЯЩАЯ в MVP** — пересчёт не подключён, поведение под `feature_toggles.reputation_reviews`. |
| `consents` | `../04-decisions/0020-versioned-consent-record-model.md` | **Append-only версионированный журнал согласий** (ADR-0020, миграция 0029) — законное доказательство (ст.9 ч.1 ФЗ-152): версия текста + отметка времени + утвердительное действие. Новая строка **замещает** предыдущую (форма supersede из ADR-0011); **текущее согласие = последняя строка по `(user_id, consent_type)`**; UPDATE/DELETE блокируются `trg_consents_immutable`. Колонки: `user_id` (**субъект данных**), `consent_type` ∈ {`CONTACT_DISTRIBUTION` (действует, ст.10.1), `MARKETING`, `ANALYTICS_PROFILING`, `NONESSENTIAL_COOKIES`, `REVIEW_PUBLICATION` (все зарезервированы формой, поведение отложено — `REVIEW_PUBLICATION` добавлен миграцией 0040 для согласия на публикацию отзыва, ADR-0039 §4)}, `granted` (true=выдача / false=отзыв-как-новая-строка, ст.9 ч.2), `policy_version`, `source` (`PROFILE_SETTINGS`/`REGISTRATION`/`ADMIN`/`AGENT`), плюс actor-снапшот ADR-0011/0006 `actor_id` + `actor_principal_type` (HUMAN\|AGENT — кто записал, обычно == `user_id`, отличается для оператора/AGENT от-имени). Индексы: `idx_consents_current (user_id, consent_type, created_at DESC)` + частичный agent-индекс. |
...
(см. `specs/statemachines/user_state_machine.md`). `listings.escalated_at` (миграция 0024) —
**маркер идемпотентной эмиссии SLA-эскалации** для события `Moderation.Escalated` — ставится в той же
транзакции, что и outbox-запись, так что просроченный PENDING_MODERATION-элемент эскалируется максимум
один раз; SLA-job никогда не меняет `status`/`moderation_status` (M-13/SLA-1/SLA-3); сбрасывается в NULL
при re-enqueue (M-14). Сканируется частичным индексом `idx_listings_escalation_scan` на
`(moderation_enqueued_at) WHERE status='PENDING_MODERATION' AND escalated_at IS NULL`. Гео:
...
### Связующая таблица мультиролей — `user_roles` (ADR-0022, миграция 0034)

`user_roles(id, user_id, role, actor_id, actor_principal_type, created_at)` резервирует шов
**«один аккаунт — много ролей»** (comfort-BR — один аккаунт это владелец + грумер + продавец товаров +
покупатель), чтобы при разрастании role-gated функций предложений (ADR-0014/0016) не потребовалась
**одновременная переделка** одинокого `users.role` и каждого чтения авторизации. `role` использует
канон из 7 ролей (та же лексика, что и `users.role`); `UNIQUE(user_id, role)`
(`uq_user_roles_user_role`); FK `user_id → users(id) ON DELETE CASCADE`; индекс `idx_user_roles_user`.
Снимок `actor_id` + `actor_principal_type` (HUMAN|AGENT) фиксирует, **кто выдал** роль — выдать роль
может ИИ-агент (ADR-0006/0011); роль ортогональна `principal_type` и `role_in_org`.

**Спящая в MVP (ADR-0022 правила 2 и 5):** `users.role` остаётся единственной авторитетной
**основной/активной-по-умолчанию** ролью и **единственным источником авторизации** — ни один код не
читает `user_roles` для авторизации, поэтому строка здесь **не даёт никаких прав**. Миграция 0034
делает backfill по одной строке на пользователя из текущего `users.role`; существующий путь смены роли
администратором поддерживает консистентность связующей таблицы аддитивной **записью-при-записи**
(идемпотентная выдача в той же транзакции). **Самозаявление роли (self-claim), отзыв и чтение
авторизации из связующей таблицы намеренно отложены** до первого слайса, поставляющего role-gated
функцию предложений (правило 5); самозаявленная *регулируемая* роль publish-gated по своему уровню
верификации ADR-0016 (само заявление не даёт ничего регулируемого).
