...
| `consents` | `../04-decisions/0020-versioned-consent-record-model.md` | **Append-only версионированный журнал согласий** (ADR-0020, миграция 0029) — законное доказательство (ст.9 ч.1 ФЗ-152): версия текста + отметка времени + утвердительное действие. Новая строка **замещает** предыдущую (форма supersede из ADR-0011); **текущее согласие = последняя строка по `(user_id, consent_type)`**; UPDATE/DELETE блокируются `trg_consents_immutable`. Колонки: `user_id` (**субъект данных**), `consent_type` ∈ {`CONTACT_DISTRIBUTION` (действует, ст.10.1), `MARKETING`, `ANALYTICS_PROFILING`, `NONESSENTIAL_COOKIES` (зарезервированы формой, поведение отложено)}, `granted` (true=выдача / false=отзыв-как-новая-строка, ст.9 ч.2), `policy_version`, `source` (`PROFILE_SETTINGS`/`REGISTRATION`/`ADMIN`/`AGENT`), плюс actor-снапшот ADR-0011/0006 `actor_id` + `actor_principal_type` (HUMAN\|AGENT — кто записал, обычно == `user_id`, отличается для оператора/AGENT от-имени). Индексы: `idx_consents_current (user_id, consent_type, created_at DESC)` + частичный agent-индекс. |
