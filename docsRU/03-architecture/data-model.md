...
| `reviews` | `specs/18-reputation.md` · [`../04-decisions/0039-reputation-storage-model.md`](../04-decisions/0039-reputation-storage-model.md) | **Append-only, один-ТЕКУЩИЙ-на-(сделку, направление) отзыв с доказательством сделки** (ADR-0039, миграция 0040) — `rating` 1..5 + опциональный `body`, написанный стороной **CONFIRMED** `confirmed_sales` о другой (FK `confirmed_sale_id` ON DELETE CASCADE; нет подтверждённой сделки ⇒ нет отзыва). **Текущий / замещение разрешаются по монотонному `seq BIGINT GENERATED ALWAYS AS IDENTITY` (урок миг. 0036), НИКОГДА не `created_at`** (tie-break на случайном UUID при равном µs — это fail-open баг, исправленный 0036 для consents). Double-blind раскрытие через `is_visible` (DEFAULT FALSE, форк 2). `direction` ∈ {`BUYER_ON_SELLER`,`SELLER_ON_BUYER`}; `moderation_status` ∈ {`PENDING`,`APPROVED`,`REJECTED`,`CHANGES_REQUESTED`} (тело модерируется как контент объявления, ADR-0040 §2). **Один head на (сделку, направление) через частичный уникальный `uq_reviews_current_per_direction … WHERE superseded_by_id IS NULL`** (форма transfer INV-4 — простой 3-колоночный UNIQUE неэффективен, NULL-ы различны); напряжение forward-`superseded_by_id` vs append-only отмечено для слайса поведения (DESIGN NOTE миграции 0040). Фасет-колонки (`facet_description_accuracy`/`facet_communication`/`facet_as_described`) зарезервированы **СПЯЩИМИ** (форк 5 — без CHECK; шкала решается в фазе фасетов). Party-FK (`reviewer_user_id`/`subject_user_id`) `ON DELETE SET NULL` = ФЗ-152 псевдонимизация-автора-с-сохранением-рейтинга (форк 8, под юр. одобрением). Actor-снапшот `actor_id` + `actor_principal_type` (HUMAN\|AGENT — авторство ВЫКЛ, ADR-0040 §3). Неизменяемость через **переиспользованный** `trg_reviews_immutable`. **СПЯЩАЯ в MVP** — эндпоинтов нет, поведение под `feature_toggles.reputation_reviews`. |
