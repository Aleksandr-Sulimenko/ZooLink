...
## 3. Текстовые вайрфреймы — ключевые экраны

Самые нагруженные задачи по спеке. Сначала ядро: **(A) Лента поиска маркетплейса** (главная задача покупателя), **(B) Создание объявления** (главная задача продавца), **(C) Детали объявления + «Показать контакты»** (экран конверсии, уникальный из-за отсутствия чата). Затем экраны продавца/оператора: **(D) Мои объявления**, **(E) Очередь модерации и проверка**, **(F) Добавить / редактировать животное**.
...
### D. Мои объявления (контрол-центр продавца)
*Цель пользователя: видеть все свои объявления по статусам и делать правильное следующее действие.*

**Layout (Десктоп):**
- **[Top Bar]** стандартный.
- **[Табы статусов]** `Все · Черновик · На проверке · Активные · Проданы · Истекли · Деактивированы` (счётчики на табах).
- **[Контент — Таблица]** колонки: фото+заголовок, рынок, **чип статуса** + **бейдж модерации**, цена, просмотры, показы контактов, обновлено. Действия строки адаптируются к статусу: Draft → *Редактировать / Отправить*; На проверке → *(только чтение)*; Active → *Отметить проданным / Деактивировать / Продвинуть(gated)*; **Changes requested → *Редактировать и переотправить*** (выделено); Expired → *Продлить*.
- Массовый выбор для деактивации.

**Layout (Мобильный):**
- Табы статусов — горизонтально скроллящийся ряд чипов; строки → **карточки** (фото, заголовок, чип статуса, 2 метрики, меню `⋯`).
- 💡 **UX-улучшение:** выводить элементы `CHANGES_REQUESTED` наверх с инлайн-заметкой модератора, чтобы петля «исправил-переотправил» была очевидной и быстрой.

### E. Очередь модерации и проверка (Модератор)
*Цель модератора: быстро и единообразно разбирать очередь, с журналом аудита.*

**Layout (Десктоп):**
- **[Top Bar]** стандартный + **фильтр рынка** `Pet | Livestock`.
- **[Слева — Список очереди]** строки: миниатюра, заголовок, вид, **время ожидания** (SLA), отправитель/орг. Сортировка старые-первыми.
- **[Справа — Панель проверки]** выбранное объявление целиком (фото, все поля, связанное животное); контролы решения: **Approve** (primary) · **Reject** (причина обязательна, из списка) · **Request changes** (причина + заметка). Ниже: **append-only история решений** этой сущности (только чтение).
- После решения элемент уходит из очереди; следующий авто-фокусируется.

**Layout (Мобильный):**
- Один столбец: карточки очереди → тап → полный экран проверки → sticky нижний бар решения (Approve / Reject / Changes). Выбор причины — bottom-sheet.
- 💡 **UX-улучшение:** горячие клавиши на десктопе (A/R/C) и авто-переход к «следующему» держат высокообъёмную модерацию быстрой; таймер SLA меняет цвет к лимиту.

### F. Добавить / редактировать животное
*Цель пользователя: точно зарегистрировать животное; оно затем питает объявления и матчинг.*

**Layout (Десктоп):**
- **[Контент — Форма]** H1 «Новое животное». Поля: вид (дропдаун), порода (автокомплит) **или** свой текст породы (ровно одно — XOR), пол, дата рождения, кличка, окрас, чип/тату (опц.), описание. Под-секции (сворачиваемые): **Health Records** (события: type/detail/date/provider), **Reproductive Data** (для самок). Тогл владения орг (если действует за орг).
- Sticky-футер: **Сохранить**. Неизменяемые поля (вид, пол, ДР, порода) явно помечены как заблокированные после создания (ADR-0004 / триггер БД).

**Layout (Мобильный):**
- Одноколоночная форма; Health/Reproductive записи — раскрывающиеся списки со строкой «＋ Добавить запись».
- 💡 **UX-улучшение:** XOR породы (`breed_id` vs свой текст) — частый источник ошибок — используй единый контрол «Порода» с автокомплитом, предлагающим *«Не нашли? Введите своё»*, чтобы пользователь не сталкивался с двумя конкурирующими полями.
