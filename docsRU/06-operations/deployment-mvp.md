...
1. **Клонировать и сконфигурировать** — `deploy/gen-env.sh` — **единственный** документированный путь провижининга
   ...
   > что значит, какой формы. Дословное копирование даёт секреты-заглушки `__change_me__`, поэтому старт с
   > `NODE_ENV=production` **падает** в `backend/src/config/env.validation.ts`. `cp .env.example .env` делать
   > **не надо**. Обе половины проверяются: CI-job `edge-smoke` провизионирует env через `deploy/gen-env.sh` и
