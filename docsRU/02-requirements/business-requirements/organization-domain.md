...
- **Пользователь организации**: Пользователь (аккаунт на платформе), affiliated с одной или несколькими организациями, имеющий определённую роль (например, ВЛАДЕЛЕЦ, АДМИНИСТРАТОР, СОТРУДНИК, ВЕТЕРИНАР, МОДЕРАТОР).
...
- Пользователи могут быть affiliated с несколькими организациями (например, ветеринар, работающий в двух клиниках).
...
- **Производительность**: Получение организации с её филиалами и affiliated пользователями должно занимать менее 300 мс.
...
- **Безопасность**:
  ...
  - Пользователи могут создавать объявления только для организаций, с которыми они affiliated (проверка через `organization_users`).
- **Конфиденциальность**:
  - Контактные данные организации (телефон, email) показываются только если организация выбрала их делиться.
  - Личные данные пользователейprotected; объявления организации не раскрывают контактную информацию отдельного создателя, если организация не решит раскрыть её.
...
| `name_ru` | VARCHAR(200) | Да | Русское название |
| `name_en` | VARCHAR(200) | Нет | Английское название |
...
- Название組織ции должно быть уникальным (регистронезависимо) внутри платформы, чтобы избежать дубликатов.
- ИНН, если предоставлен, должен быть уникальным (разрешён NULL).
- Пользователь должен иметь активную affiliated запись (`organization_users`) с организацией, чтобы создавать объявление от её имени.
...
sequenceDiagram
    ...
    Founder->>Frontend: Заполняет название орг., ИНН, адрес и т.д.
    ...
    Founder->>Frontend: Выбирает существующего пользователя (по email/telephone), выбирает роль СОТРУДНИК
    ...
    Backend->>Database: Валидация, что пользователь affiliated с организацией, создаёт запись объявления с organization_id/branch_id
...
- **Предположение**: Большинство организаций будет иметь один головной офис и可能 несколько филиалов; модель поддерживает любое количество филиалов.
- **Предположение**: Контроль доступа на основе ролей (RBAC) будет реализован на уровне приложения для обеспечения разрешений на управление организацией/филиалом.
- **Открытый вопрос**: Следует ли позволить организации быть помеченной как "подтверждённая" (например, с бейджем) после ручной подтверждения? (Планируется на Фаза 2+)
- **Предположение**: Поля `inn` и `kpp` специфичны для российского законодательства; при международном расширении они могут стать необязательными или заменены на общий налоговый идентификатор.
- **Открытый вопрос**: Нужно ли хранить геокоординаты (широта/долгота) для филиалов, чтобы обеспечить более точный гео-поиск? (Можно добавить позже через отдельную таблицу или расширение ветвей с PostGIS point).

## Связанные домены
- **Домен идентичности**: Таблица `users` предоставляет отдельные учётные записи, которые affiliated с организациями.
...
- `POST /organization-users` (affiliate пользователя с организацией)
