...
- Обязательные поля при создании:
  - `name_localized` (локализованные названия, например, {"en": "Name", "ru": "Название"}) — **должна присутствовать хотя бы одна локаль; организацию нельзя создать без названия** (GAP-BA-006: выравнивает правило создания к таблице модели данных ниже, где `name_localized` имеет `Required: Да`).
...
> **(role-canon sync, normative) — `role_in_org` сужен до канона из 4 ролей.**
> **ЧТО:** Удалён `MODERATOR` из `role_in_org`; канон — `{OWNER, ADMIN, STAFF, VET}` согласно `database_schema.sql`
> и `docs/specs/security/rbac-matrix.md` (ADR-0011).
> **ПОЧЕМУ:** GAP-TRACE-008 — миграция схемы уже сузила `role_in_org` до 4 значений; в BR была перечислена роль
> (`MODERATOR`), которой нет в колонке.
> **ПОЧЕМУ ТАК ЛУЧШЕ для проекта в целом:** чистое разделение **платформенной** модерации (это `users.role`) и ролей
> **членства в организации** убирает неоднозначность RBAC; org-уровневая модерация, если когда-либо понадобится — вопрос Фазы 2.
...
- **Статус контракта (B9, GAP-011)**: эндпоинт org-агрегата `GET /organizations/{id}/analytics` **определён как контракт** (`organization-api.yaml`, `OrganizationAnalytics`) — счётчики сейчас (`totalListings`, `countsByStatus`, `countsByMarket` по ADR-0002, `totalViews`, `totalContactReveals`) с **series-ready** полем `series` (`x-phase:2`, аддитивно). Реализация запланирована на фронтенд-фазу (форма сейчас, поведение потом — решение владельца #6). См. `API_CONVENTIONS.md` §16.
...
| `role_in_org` | VARCHAR(20) | Да | Роль: OWNER, ADMIN, STAFF, VET (канон из 4 ролей согласно `database_schema.sql` / `rbac-matrix.md`; MODERATOR — платформенная операторская роль, **не** роль членства в организации) |
