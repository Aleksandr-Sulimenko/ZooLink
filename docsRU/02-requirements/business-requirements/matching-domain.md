...
## Объём MVP против Фазы 2 (нормативно — согласовано со spec 05)
> **ЧТО:** в MVP подбор реализован как **stateless eligible-set поиск**, а не как ранжированный
> скоринг. Кандидат показывается тогда и только тогда, когда он проходит жёсткие предикаты из
> [`specs/05-matching-domain.md` §"Breeding eligibility"](../../specs/05-matching-domain.md)
> (тот же вид · противоположный пол · оба INTACT · оба видимы в поиске разведения · оба активны ·
> разные владельцы · в пределах радиуса). В MVP **нет** таблиц `matches` / `match_history` /
> `match_feedback`; их API-поверхность (`GET /matching/{id}`, `GET /matching/history`,
> `POST /matching/{id}/feedback`) и все поля скоринга (`compatibilityScore`, `scoreBreakdown`,
> `minScore`) помечены `x-phase: 2` и null/отсутствуют в MVP (см. `matching-api.yaml`).
> **ПОЧЕМУ:** контракт и модель данных не должны обещать движок скоринга, который MVP не строит;
> `compatibilityScore`, возвращённый реальным числом, был бы тихой ложью doc↔code. Право на подбор —
> чистый предикат над существующими полями животного и не требует новой персистенции.
> **ПОЧЕМУ ТАК ЛУЧШЕ:** eligible-set forward-compatible — Фаза 2 добавляет ранжирование *поверх* того
> же набора предикатов и тех же форм ответа (аддитивные `compatibilityScore`/`scoreBreakdown`),
> поэтому переписывание контракта, схемы или actor-модели не требуется (IMPLEMENTATION_PLAYBOOK §5).
> **Разделы 2 (Факторы сопоставления и веса), 5 (скоринг вспомогательной репродукции), концепт
> Compatibility Score и концептуальная `matches`/score `Модель данных` ниже описывают дизайн
> Фазы 2**; они сохранены как forward-дизайн, а не поведение MVP.
...
- Животные должны быть **одного вида**; **порода может не совпадать** — совпадающий `breed_id` в MVP является *повышением в ранжировании*, а не жёстким фильтром, поэтому **межпородное сопоставление разрешено в MVP** (канонический источник: [05-matching-domain.md](../../specs/05-matching-domain.md), раздел «Breeding eligibility (MVP — normative)», предикат 1 + примечание о породе). Опциональное продвинутое ограничение «только та же порода» зарезервировано для Фазы 2+.

> **Примечание о выравнивании (GAP-BA-003, аудит 2026-06-30).** **WHAT:** правило разведения изменено с «одного вида **и породы** (межпородное зарезервировано для Фазы 2+)» на «одного вида; порода — повышение в ранжировании, **межпородное разрешено в MVP**». **WHY:** прежний BR противоречил проверенной нормативной спецификации [05-matching-domain.md](../../specs/05-matching-domain.md) (порода явно — *повышение в ранжировании, а не жёсткий фильтр*; предикат 1 проверяет только вид). **WHY-BETTER:** это исправление *в сторону* канонической спецификации (уровень истины 5), устраняющее противоречие doc↔spec без изобретения области; ограничение «только та же порода» напрасно подавляло бы совпадения MVP и не соответствует предикатам отбора.

### 2. Факторы сопоставления и веса *(Фаза 2+ — движок скоринга; не в MVP eligible-set)*
...
## Концептуальная модель данных *(Фаза 2+ — таблицы `matches` и колонок скоринга нет в MVP)*
> Подбор в MVP stateless (нет таблиц `matches`/`match_history`/`match_feedback` — spec 05). Таблица
> ниже — дизайн персистенции Фазы 2, когда появится скоринг.
