...
Запись о том, что реальная сделка состоялась между двумя идентифицированными сторонами, подтверждённая **обеими** сторонами (или авто-подтверждённая при привязке к COMPLETED `ownership_transfers`). Корень доказательства-транзакции, на который обязан ссылаться каждый **Review** (FORM-first, спека 18).

**Sale Anchor (Якорь сделки)**  
Вышестоящее событие, из которого выводится **Confirmed Sale**: `TRANSFER` (COMPLETED `ownership_transfers`, сильнейший — для животных) или `LISTING_MARK_SOLD` (продавец `markSold` + встречное подтверждение покупателя) (спека 18).

**Review (Отзыв)** (`reviews`)  
Append-only запись «один на (сделку, направление)»: рейтинг + опциональный текст, оставленный одной стороной **Confirmed Sale** о другой. Требует CONFIRMED-сделки (доказательство-транзакции) (FORM-first, спека 18).
