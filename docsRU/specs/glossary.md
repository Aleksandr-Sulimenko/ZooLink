...
**Append-only, только-CONFIRMED ФАКТ** того, что реальная сделка состоялась между двумя идентифицированными сторонами (пишется автоматически при привязке к COMPLETED `ownership_transfers` или когда переговоры **Sale Confirmation** достигают CONFIRMED). Строка существует тогда и только тогда, когда сделка подтверждена (`CHECK (status='CONFIRMED')`). Корень доказательства-транзакции, на который обязан ссылаться каждый **Review** (FORM-first, спека 18; ADR-0038 §4 Дополнение, миграция 0041).

**Sale Confirmation (Подтверждение сделки)** (`sale_confirmations`)  
**Изменяемый** жизненный цикл переговоров `markSold` (`PENDING_CONFIRMATION → CONFIRMED/EXPIRED/CANCELLED/DISPUTED`) — «изменяемое состояние»-спутник неизменяемого факта **Confirmed Sale**. Достижение CONFIRMED пишет строку-факт (биусловие `chk_sale_conf_confirmed_link`). Якорь `TRANSFER` его пропускает (рождается CONFIRMED). Одни живые переговоры на листинг (спека 18; ADR-0038 §4 Дополнение, миграция 0041).

**Sale Anchor (Якорь сделки)**  
Вышестоящее событие, из которого выводится **Confirmed Sale**: `TRANSFER` (COMPLETED `ownership_transfers`, сильнейший — для животных; рождается CONFIRMED) или `LISTING_MARK_SOLD` (продавец `markSold` + встречное подтверждение покупателя через **Sale Confirmation**) (спека 18).

**Review (Отзыв)** (`reviews`)  
Append-only запись «один текущий на (сделку, направление)»: рейтинг + опциональный текст, оставленный одной стороной **Confirmed Sale** о другой. Требует CONFIRMED-сделки (доказательство-транзакции). Правка-в-grace — это новая строка, называющая своего предшественника через обратный `supersedes_review_id`; текущая разрешается view `reviews_current` (FORM-first, спека 18; ADR-0039 §3 Дополнение, миграция 0041).

**Review State (Состояние отзыва)** (`review_states`)  
**Изменяемый** статус модерации отзыва (`PENDING/APPROVED/REJECTED/CHANGES_REQUESTED`) + double-blind `is_visible` — «изменяемое состояние»-спутник неизменяемого факта **Review**; ключ `review_id`, читается через join (спека 18; ADR-0039 §3 Дополнение β, миграция 0041).

**Review Direction (Направление отзыва)**  
`BUYER_ON_SELLER` или `SELLER_ON_BUYER` — репутация **двусторонняя**; обе стороны могут оценить друг друга (спека 18).

**Reputation Aggregate (Агрегат репутации)** (`reputation_aggregates`)  
**Производная, пересчитываемая** сводка рейтинга на субъекта × на рынок (count, mean, распределение). Никогда не пишется вручную, никогда не покупается (FORM-first, спека 18).

**Double-blind release (Двойное-слепое раскрытие)**  
Отзыв ни одной из сторон не виден, пока **обе** не отправили **или** окно отзывов не закрылось — снижает предвзятость мести/взаимности (рекомендуемая политика видимости) (спека 18).

**Unconfirmed / weak sale (Неподтверждённая / слабая сделка)**  
`markSold`, так и не подтверждённый встречно (EXPIRED-подтверждение). *Сигнал* захвачен, но он **не разблокирует отзывы** (спека 18).
...
**OfferingRef** (полиморфная ссылка на предложение)  
Пара `(offering_type, offering_id)` в `favorites` и `saved_searches` (ADR-0014, миграция 0032), чтобы discovery/избранное/сохранённый поиск ключевались на **предложение**, а не на listing-only FK. Сегодня `offering_type` ограничен `ANIMAL_LISTING` (== `listing_id`); это аддитивный anti-god-table шов **без** FK (цель полиморфна). В сохранённом поиске `offering_type` — дискриминатор, а `offering_id` остаётся NULL (поиск — это запрос, а не одно предложение).

**derived-`market` cache (кэш производного рынка)**  
`listings.market VARCHAR(9)` (`pet|livestock`, ADR-0018 Amendment / миграция 0033) кэширует **производный** рынок — `species.market`, достигаемый через животное — чтобы discovery / модерация / скан SLA-эскалации читали рынок с **нулём** кросс-агрегатных джойнов (Wave D8/D8b). Кэширует *производный* рынок, **не** назначенный тег `market_scope` (rule 7 ADR-0015 соблюдается). Репозиторный grep-гейт (`check-no-raw-market-join.sh`) запрещает сырые чтения рынка `animals ⋈ species` вне агрегата animal.

**geo_anchor** (geo-anchor, зарезервировано)  
Первоклассное, offering-agnostic **начало поиска** для find-nearby discovery. Сегодня его точечная форма — триплет `lat`/`lng`(+`radius_km`) (listings-api, спека 07 round-6, согласовано в Wave D7); более богатые якоря (зона обслуживания, PostGIS) — зарезервированный gated поздний swap и сейчас в runtime ничего не дают.

**monetization_type** (зарезервировано, spec-only)  
Дискриминатор модели монетизации предложения `{LEAD_GEN, SUBSCRIPTION, TAKE_RATE, NONE}` (SCREAMING_SNAKE), зарезервирован на контракте предложения, чтобы модель переключалась без переписывания (ADR-0014 §Amendment 2026-07-05 / Wave D9). **Spec-only:** конверт read-model пока НЕ должен его нести, а сама *модель* монетизации отложена до явного обсуждения с владельцем.
