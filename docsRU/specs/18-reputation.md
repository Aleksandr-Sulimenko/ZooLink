...
|---|---|---|
| ∅ → CONFIRMED | `OwnershipTransfer.Completed` | anchor=`TRANSFER`; стороны = from/to передачи; авто-подтверждение (передача уже двусторонняя по ADR-0013) |
...
Feature: Двусторонняя подтверждённая сделка
  ...
  Scenario: Авто-подтверждение из завершённой передачи животного (сильнейший сигнал)
    ...
    When обрабатывается событие OwnershipTransfer.Completed
...
1. **`confirmed_sales` пишется пассивно при `OwnershipTransfer.Completed`** (anchor=`TRANSFER`,
   авто-CONFIRMED, в той же транзакции, что и передача). **Это шов наивысшей ценности из спящих** — это
   *сигнал подтверждённой сделки, который мы сегодня безвозвратно теряем* (та же логика reserved-first,
   что у `view_count` D1). Отзывы выключены; *источник истины* начинает накапливаться уже сейчас.
2. **Таблицы `confirmed_sales` + `reviews` + `reputation_aggregates`** созданы спящими, с
   переиспользованием append-only-триггера. Эндпоинты чтения/записи отзывов не подключены.
3. **Строки `feature_toggles`** засеяны off/0 %: `reputation_reviews` (авторство/чтение отзывов),
   `sale_buyer_confirmation` (путь встречного подтверждения listing-markSold). Форма как у
...
## 13. Открытые вопросы / Допущения (БИЗНЕС-развилки ВЛАДЕЛЬЦА — с рекомендацией каждая; не блокируют проектирование)
