...
> Состояние БД: **31 таблица**, миграции `0001`–`0012` идемпотентны (`0011` = reference-seed, `0012` = outbox relay delivery-state: attempts/last_error/next_attempt_at/dead_lettered_at + снят ошибочный updated_at-триггер на `outbox_events`), ~два десятка инвариантов enforced. Reference + reasons/templates сидятся `npm run seed` (идемпотентно).
