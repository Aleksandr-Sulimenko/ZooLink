...
ADR (`docs/04-decisions/NNNN-*.md`) → **`database_schema.sql`** (валидируется на живом PG) → **`API_CONVENTIONS.md`** (`docs/03-architecture/api-contracts/`) → доменные спеки (`docs/specs/NN-*.md`) → baseline'ы. Глоссарий: `docs/specs/glossary.md`. ERD-канон: `ZooLink_ERD.mmd`.
...
> Состояние БД: **31 таблица**, миграции `0001`–`0013` идемпотентны (`0011` = reference-seed, `0012` = outbox relay delivery-state: attempts/last_error/next_attempt_at/dead_lettered_at, `0013` = updated_at-триггеры выставляются по факту наличия колонки — снят ошибочный триггер с `outbox_events`/`animal_ownership_history`/`messages`, добавлен недостающий на `digital_assets`, `0014` = снят избыточный `idx_outbox_unprocessed`), ~два десятка инвариантов enforced. Reference + reasons/templates сидятся `npm run seed` (идемпотентно).
