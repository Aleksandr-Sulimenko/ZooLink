# Performance Non-Functional Requirements: ZooLink

## Purpose
Defines performance requirements to ensure responsive user experience, scalability, and efficient resource utilization under expected load conditions.

## Scope
Applies to all system components: backend APIs, frontend applications, database queries, file operations, and integrations.

## Performance Goals & Metrics
### Response Time Targets
- **API Endpoints**:
  - 95% of requests < 1s under normal load
  - 99% of requests < 2s under normal load
  - Maximum 95th percentile < 3s during peak load
- **Specific Critical Endpoints**:
  - Geo-search listings: < 1.5s for 95% of queries (<100km radius)
...
### Throughput & Capacity
- **Concurrent Users**: 
  - Support 500 concurrent active users without degradation
  - Handle 1000 concurrent sessions (including idle tabs)
- **Requests Per Second (RPS)**:
  - Sustainable: 50 RPS average
  - Peak: 200 RPS for 5-minute bursts
  - Burst: 500 RPS for 30-second spikes
- **Data Volume**:
  - Support 100K total listings (active+archived) without degradation
...
- **CPU**: 
  - Average utilization < 60% under normal load
  - Peak utilization < 85% during traffic spikes
- **Memory**:
  - Average utilization < 70% under normal load
  - Peak utilization < 85% during traffic spikes
...
- **Storage**:
  - Disk space utilization < 70% to allow for growth and maintenance
