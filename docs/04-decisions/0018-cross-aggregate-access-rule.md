...
> **Code fulfils Part-2 — DONE (Wave D8, 2026-07-06, code-only, no migration).** All market readers (listing / moderation / scheduler) now read the derived `listings.market` cache (D3), zero market joins: (1) listing discovery CTE — `l.market` (the `animals` join now added only when a species_id/breed_id filter is present); (2) `listing.marketOf` — deleted, callers read `listing.market` off the loaded row; (3) `moderation.marketOf` — deleted, reads `listing.market`; (4) moderation queue base CTE — `l.market`; (5) the SLA-escalation scan (`lib/scheduler/moderation-escalation.service.ts`) — `l.market` (see D8b below). The **grep-gate is active**: `scripts/check-no-raw-market-join.sh` + blocking CI job `market-decoupling` fail on any raw `FROM animals`/`JOIN species`. Sole documented exception (inline `grep-allow-market-join` marker): the queue CTE keeps `JOIN species` ONLY to project `species.code` for display — market itself is `l.market`. Parity is proven by the unchanged discovery/market/queue e2e suites (L2-1 market-no-leak lives) + a stale-cache negative test.
>
> **D8b (2026-07-07, code-only, no migration) — 5th reader + gate widened.** *WHAT:* the SLA-escalation scan (worker-only) still read `s.market` via a raw `animals ⋈ species` join to pick the pet-4h/livestock-6h threshold and tag the `Moderation.Escalated` event, and it sat OUTSIDE the D8 gate (which scanned only `listing`/`moderation`). It now reads `l.market`, and the gate scans **all of `backend/src` except the animal aggregate** (`modules/animal` = AnimalService + the intra-aggregate transfer flow, the sanctioned owner of animal↔species) plus marker lines. *WHY:* reviewer found the reader; a market reader outside the gate can silently re-introduce the cross-aggregate coupling / market-leak drift ADR-0018 breaks. *WHY-BETTER:* the invariant "every external market reader reads the cache" is now enforced repo-wide, not just in two modules — future readers (any new module/scheduler) are caught by CI. Parity: identical thresholds/tags (cache == species.market per D3), proven by the unchanged SLA-boundary e2e (pet 8h, livestock 12h) plus a new drift test showing the threshold + tag follow `l.market`, not the species.
