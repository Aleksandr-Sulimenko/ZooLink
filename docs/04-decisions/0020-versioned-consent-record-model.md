...
**Status**: Proposed — ready; awaiting owner nod on the bundled **Owner-Decisions OD-1/OD-2/OD-3** below. The *model* (append-only versioned consent log, ADR-0011 supersede shape) is a straight application of already-ratified patterns; the open items are product-granularity + a user-visible default flip, so the honest status is Proposed with a one-pass ratification block.
**Date**: 2026-07-03
