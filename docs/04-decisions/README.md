...
- [ADR-0038: Confirmed-Sale record of truth — a first-class append-only entity (not a projection), polymorphic subject, event-emitting](0038-confirmed-sale-record-of-truth.md) *(Accepted 2026-07-09 — spec 18-reputation §12 items 1/7/9 (ConfirmedSale.*); AUDIT4 P3-1. `confirmed_sales` is its own append-only entity anchored to a COMPLETED transfer or listing markSold (not a view); polymorphic `offering_type` (mig-0032 seam) + derived `market`; `ConfirmedSale.*` outbox events; form-now dormant transfer-anchored capture behind `sale_buyer_confirmation`/`reputation_reviews`. Owner Open-Q: capture `amount_minor`?)*
- [ADR-0039: Reputation storage model — materialised per-(subject, market) aggregate over append-only reviews; erasure keeps the rating](0039-reputation-storage-model.md) *(Accepted 2026-07-09 — spec 18-reputation §12 items 2/3/8/9 (Review.*). Materialised `reputation_aggregates` (incremental O(Δ), never on-read/hand-written); per-market PK over one identity (fork 6, ADR-0002 structural); reviews append-only resolved on monotonic `seq` (mig-0036 lesson); erasure = pseudonymise-author-keep-rating (fork 8, legal-gated). Owner Open-Q: ФЗ-152 basis to keep an erased user's rating + free-text body policy)*
- [ADR-0040: Reputation trust integrity & governance — verification-tier orthogonal (weighting reserved), disputes as a `content_report` subtype, agent-as-review-moderator (not reviewer)](0040-reputation-trust-integrity-governance.md) *(Accepted 2026-07-09 — spec 18-reputation §12 items 4/5/6; AUDIT4 P3-1/§8. Verification tier orthogonal in MVP (dormant weighting hook), anti-Sybil = proof-of-transaction + quota + anomaly→moderation; review disputes/abuse reuse moderation as a `content_report` subtype (no parallel court); agent-as-review-moderator via ADR-0037 scope, agent-as-reviewer deferred/NO; reputation never monetised (fork 7). Owner Open-Qs: ever allow agent-authored reviews? activate tier-weighting on fraud data?)*
- [ADR-0041: Refresh-cookie `SameSite=Strict` binds ZooLink to a single-site edge topology (a cross-site SPA would require revisiting)](0041-refresh-cookie-samesite-single-site-topology.md) *(Accepted 2026-08-06 — AUDIT5 frontend-engineer Б-2/Б-3/§7.2, surfaced by the F1a base-path pack. `Strict` = strongest CSRF posture and free under ADR-0009 same-origin edge; it silently fixes a single-site (same registrable domain) front end. Keep Strict now — do NOT weaken preemptively. A same-site subdomain split needs only the seamed `CORS_ORIGINS`; a **cross-site** SPA (different registrable domain / cross-site CDN) MUST reopen this ADR to relax SameSite→Lax/None;Secure AND add a compensating CSRF token. No code/schema change.)*
