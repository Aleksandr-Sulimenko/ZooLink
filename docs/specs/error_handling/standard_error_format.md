> ⛔ **SUPERSEDED 2026-08-09 — ЭТОТ ДОКУМЕНТ ОПИСЫВАЕТ ФОРМАТ, КОТОРОГО В КОДЕ НИКОГДА НЕ БЫЛО.**
> status: superseded
> superseded_by: `docs/03-architecture/api-contracts/API_CONVENTIONS.md` §4 (RFC 7807 `Problem`) +
> `docs/04-decisions/0043-error-response-body-allow-list.md`
>
> Решение владельца 09.08.2026: **признать устаревшим** (не переписывать код под этот документ).
...
>   имена другие, поэтому переносить сюда нечего.
> ПОЧЕМУ ВЫБРАЛИ RFC 7807, а не этот документ: это внешний стандарт с готовым тулингом, он уже стоит
> в 12 EN-контрактах и 12 RU-зеркалах (24 места) и на нём держится вся приёмка; числовые диапазоны
> 4000–5999 — наш велосипед, не реализованный ни разу за 8 месяцев.
> ЧЕСТНО О ЦЕНЕ ЭТОГО ПРИЗНАНИЯ: документ-контракт **полгода описывал несуществующую систему**, и на
> него ссылались `API_CONVENTIONS.md`, `nfr/availability.md` (EN+RU), `specs/11-organization-domain.md` —
> то есть читатель, пришедший по ссылке, получал выдумку. Отсюда заказ владельца: найти ПОДОБНЫЕ
> артефакты по всему дереву (ревизия «канон против кода» + прибор, чтобы класс не возвращался).
