...
| **INV-C5-4** (rate-limit) | Claim-code mint **and** transfer-initiate are Redis rate-limited per principal → `429 RATE_LIMITED` + `Retry-After`. | **Established-precedent** per-principal, per-hour **manual Redis `INCR`** gate (uniform with the contact-reveal limiter in `listing.service`; first hit sets the hour TTL, a count over the limit → `429 RATE_LIMITED` with `retryAfter` = key TTL, rendered as the `Retry-After` header by the problem filter). Chosen over `@nestjs/throttler` for consistency with the existing reveal limiter and because our filter carries a real `Retry-After` — observably `429`+`Retry-After` either way (gate-accepted deviation). |
