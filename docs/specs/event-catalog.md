...
| `SavedSearch.Matched` | SavedSearch | notification worker (`SavedSearchMatchConsumer`, in the per-pair insert-won tx) | savedSearchId, listingId, subjectUserId, market, matchedAt | **none yet** (analytics-only; dormant). Later: match→view→contact demand-side funnel (data-analyst). Emitted **exactly once per (saved_search, listing) pair** — only when the alert row is newly inserted (SS-M4-consistent). |
...
> **(round-N, normative — H4 follow-ups: `SavedSearch.Matched` analytics event + `q` substring match now live, Slice H4 follow-ups) WHAT:** two updates to the H4 note above. **(1)** the H4 note stated "no `SavedSearch.Matched` event … needed" for *dedup* — that remains true (dedup is the per-pair notification `idempotency_key`), but `SavedSearch.Matched` is now **additionally** emitted as an **analytics** event (row added to §2), NOT as a dedup mechanism. It is produced by `SavedSearchMatchConsumer` **inside the same transaction as — and only when — the per-pair notification row is newly inserted** (the `ON CONFLICT DO NOTHING` insert-won branch): a redelivered `Listing.Activated` re-runs the idempotent INSERT (0 rows affected) → no event, so the analytics event is **exactly-once per pair**, SS-M4-consistent. **No consumer subscribes** (dormant; feeds the future match→view→contact funnel a data-analyst will spec). **(2)** the H4 note's "`q` free-text is NOT evaluated" is **superseded** — `q` is now evaluated server-side as a case-insensitive **substring** match over the listing's both-locale title+description (spec 07 **SS-M2**, updated in place); it is a substring (no stemming/ranking), needs no schema change, and a `q`-bearing search still must satisfy its other filters. **WHY:** the analytics event is the seam the demand-side funnel hangs off (the `notification_logs` rows alone are hard to join into a funnel), and a text search that ignored its text term was the surprising H4 gap. **WHY-BETTER-for-the-whole-project:** emitting the event in the insert-won tx reuses the outbox atomic-with-DB-change guarantee for exactly-once with **zero new dedup store**, and building `q` as an in-memory substring keeps the slice **zero-DDL** while leaving a stemmed-FTS upgrade behind an ADR seam. RU mirror updated.
