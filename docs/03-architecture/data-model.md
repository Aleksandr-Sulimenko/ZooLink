...
| `confirmed_sales` | `specs/18-reputation.md` · [`../04-decisions/0038-confirmed-sale-record-of-truth.md`](../04-decisions/0038-confirmed-sale-record-of-truth.md) | **Append-only record-of-truth that a real deal happened** (ADR-0038, migration 0039) — the proof-of-transaction root reputation (ADR-0039) hangs off; a first-class entity, NOT a projection (it holds new facts the sources lack: buyer counter-confirm, dispute, actor-snapshot). Anchored to a COMPLETED `ownership_transfers` (auto-CONFIRMED, written **in the same tx** as the transfer accept so the strongest sale signal is never lost) or a listing `markSold` (buyer counter-confirmation, deferred behind `feature_toggles.sale_buyer_confirmation`). Immutable via the **reused** `trg_confirmed_sales_immutable` (do NOT invent a second immutability path). Polymorphic subject `offering_type`/`offering_id` (migration 0032 dialect, `ANIMAL_LISTING`-only now, widen-additively) + concrete `listing_id`/`animal_id`. Derived `market` cache (ADR-0018/0033, ADR-0002 hard split). `anchor_type` ∈ {`TRANSFER`,`LISTING_MARK_SOLD`}; `status` ∈ {`PENDING_CONFIRMATION`,`CONFIRMED`,`DISPUTED`,`EXPIRED`,`CANCELLED`} (full vocabulary reserved; the FORM slice writes only `CONFIRMED`); `UNIQUE (ownership_transfer_id)` = one sale per transfer (mirror INV-4). Actor-snapshot `actor_id` + `actor_principal_type` (HUMAN\|AGENT). `nominated_buyer_user_id` reserved form-now (markSold path). `amount_minor BIGINT` **reserved nullable, off-record default (owner 2026-07-09)** — NO code path writes it. Party FKs `ON DELETE SET NULL` (ФЗ-152 pseudonymisation). **DORMANT capture in MVP** — no consumer, no endpoint reads it; the transfer path emits `ConfirmedSale.Confirmed` in-tx (NOT `.Created` — the transfer row is born CONFIRMED with no PENDING phase). |
| `reviews` | `specs/18-reputation.md` · [`../04-decisions/0039-reputation-storage-model.md`](../04-decisions/0039-reputation-storage-model.md) | **Append-only, one-current-per-(sale, direction) proof-of-transaction review** (ADR-0039, migration 0040) — `rating` 1..5 + optional `body`, authored by a party of a **CONFIRMED** `confirmed_sales` about the other (FK `confirmed_sale_id` ON DELETE CASCADE; no confirmed sale ⇒ no review). **Current / supersession resolve on the monotonic `seq BIGINT GENERATED ALWAYS AS IDENTITY` (mig 0036 lesson), NEVER `created_at`** (a same-µs tie-break on a random UUID is the fail-open bug 0036 fixed for consents). Double-blind release via `is_visible` (DEFAULT FALSE, fork 2). `direction` ∈ {`BUYER_ON_SELLER`,`SELLER_ON_BUYER`}; `moderation_status` ∈ {`PENDING`,`APPROVED`,`REJECTED`,`CHANGES_REQUESTED`} (body moderated like listing content, ADR-0040 §2). **One head per (sale, direction) via the partial unique `uq_reviews_current_per_direction … WHERE superseded_by_id IS NULL`** (transfer INV-4 shape — a plain 3-col UNIQUE is ineffective, NULLs are distinct); the forward `superseded_by_id` vs append-only tension is flagged for the behaviour slice (migration 0040 DESIGN NOTE). Facet columns (`facet_description_accuracy`/`facet_communication`/`facet_as_described`) reserved **DORMANT** (fork 5 — no CHECK; scale decided in the facet phase). Party FKs (`reviewer_user_id`/`subject_user_id`) `ON DELETE SET NULL` = ФЗ-152 pseudonymise-author-keep-rating (fork 8, legal-gated). Actor-snapshot `actor_id` + `actor_principal_type` (HUMAN\|AGENT — authoring OFF, ADR-0040 §3). Immutable via the **reused** `trg_reviews_immutable`. **DORMANT in MVP** — no endpoints, behaviour behind `feature_toggles.reputation_reviews`. |
