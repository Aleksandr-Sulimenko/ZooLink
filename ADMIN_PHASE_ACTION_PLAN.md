...
| **B0.6** 🟠 | **actor в ответах = `{actorId, principalType}`** (agent-badge) — правило API_CONVENTIONS + применить к moderation/audit-формам (C5). | architect→doc-keeper | ✅ (UNBLOCKED — ADR-0011 Accepted, snapshot live миграция 0016; правило закреплено в API_CONVENTIONS §15 + Conformance status «B0.6 done»; применено: admin-api `AuditLogEntry.actor`/`ModerationLogEntry.actor`/`ModerationActionResponse.actor`/`SystemSetting.updatedBy` = `Actor`, query `actorId` заменил `performedBy`; moderation-api `ModerationDecision.actor`+override/`ContentReport.resolvedBy`. Плоские `moderatorId/performedBy/resolvedBy/updatedBy` сняты, TODO-пометки удалены. Owner-decision #5 «решено ИИ — показывать ВСЕМ» — данные присутствуют независимо от выбора отображения) |
...
| **D4** | reference-CRUD-аудит (read-видимость → поднять в B0.6/B8) + operator-password-policy spec | GAP-006-sub | ✅ (2026-06-24, doc-keeper: spec06 EN+RU «Reference-data audit & operator security (D4)» — мутации reference пишутся в `audit_log` через `entity_id_int` (A2) и читаемы через `GET /audit/log` фильтр actor/entityType=reference-data → закрывает GAP-006-sub versioning; operator-password-policy слинкован на `security_specification.md` (12+complexity/bcrypt≥12/lockout 5×15м/TTL 15м-7д), без новой инфры. Флаг RESOLVED (2026-06-25, alpha-analyst contract-owner): `getAuditLog` + `AuditLogEntry` получили доп. `entityIdInt` (integer) рядом с UUID `entityId` (→ `audit_log.entity_id_int`, миграция 0018; «ровно одно заполнено на строку»); фильтры взаимоисключающие (оба → 400); INT reference-id теперь фильтруется. Применено в admin-api.yaml + spec06 D4 с тройкой; RU-зеркало — doc-keeper) |
