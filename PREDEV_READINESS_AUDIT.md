...
>
> ## ✅ Статус устранения — P2 закрыт (2026-06-17)
> - **Стейт-машины нарисованы:** добавлены mermaid `stateDiagram-v2` в `listing_state_machine.md` и `user_state_machine.md` (EN+RU); `ownership_transfer` получил диаграмму ещё в P0. Все провалидированы рендером.
> - **ASCII-ERD устранён:** в `data-model.md` (EN+RU) заменён ссылкой на канонический `ZooLink_ERD.mmd` (убрал дрейф дублирования).
> - **Текстовые противоречия сняты:** mvp-scope — поля отзывов помечены «будут добавлены в Фазе 2 (в схеме нет)»; гео-спека UC-GS-03 — явно: хранение saved-search = MVP, алерты = Phase 2.
>
> **Итог: P0 + P1 + P2 закрыты.** Все mermaid-модели (12 процессных + ERD + 5 стейт-машин) валидируются рендером; схема исполняется на живом PG14 (26 таблиц).
...
# ✅ ГОТОВО К РАЗРАБОТКЕ (после устранения P0/P1/P2 — 2026-06-17)

> Исходный вердикт был **⚠️ ТРЕБУЕТ ДОРАБОТКИ** (см. ниже зачёркнутый список). После проработки P0/P1/P2 спека приведена к dev-ready состоянию: данные и критичные процессы смоделированы визуально и провалидированы.

**Сторона ДАННЫХ — готова** (ERD полон: 26 сущностей, DDL исполняется на живом PG14, миграция идемпотентна). **Сторона ПРОЦЕССОВ — готова для критичных потоков** (платёж, передача владения, модерация имеют BPMN-схемы с ветками ошибок и актёрами; все стейт-машины нарисованы).

### Чек-лист устранения (что было → сделано):

**P0 — блокеры процессов → ✅ ВСЕ ЗАКРЫТЫ:**
1. ✅ BPMN платёжного потока → `specs/14-payment-domain.md` (актёры + success/fail/timeout/dispute/refund).
2. ✅ BPMN передачи владения → `statemachines/ownership_transfer_state_machine.md` (swimlane + state-диаграмма).
3. ✅ Стейт-машины платежа и уведомления → `statemachines/payment_state_machine.md`, `notification_state_machine.md`.
4. ✅ Схема модерации с ветвлениями → `specs/12-moderation-domain.md` (approve/reject/changes/SLA-timeout, актёры).

**P1 — пробелы данных → ✅ ВСЕ ЗАКРЫТЫ:**
5. ✅ Favorite → таблица `favorites` + ERD.
6. ✅ SavedSearch → MVP-модель `saved_searches` (алерты Phase 2).
7. ✅ UserReport → таблица `content_reports` (MVP).

**P2 — визуализация и гигиена:**
8. ✅ 3 стейт-машины нарисованы как mermaid (listing/user/ownership).
9. ✅ Sequence-диаграммы animal/organization/admin переписаны: были **сломаны** (не рендерились — скобки в participant, многострочные сообщения), теперь валидны + выровнены со схемой (ACTIVE/DEACTIVATED/moderation_decisions/users.status) + добавлены alt/else error-ветки (403/404/409/422, conflict, immutable-field, duplicate). EN+RU, рендер провалидирован.
10. ✅ ASCII-ERD в data-model.md заменён ссылкой на канон `ZooLink_ERD.mmd`.
11. ✅ Текстовые противоречия сняты (reviews-поля, скоуп saved-search).

### Итого по готовности
| Аспект | Готовность |
|---|---|
| Модель данных (ER/SQL) | 🟢 готово — 26 сущностей, DDL исполняется, миграция идемпотентна |
| Стейт-машины (логика + визуал) | 🟢 готово — 5 стейт-машин с диаграммами (listing/user/ownership/payment/notification) |
| Процессы (BPMN) | 🟢 готово — критичные потоки (платёж/трансфер/модерация) + все sequence-диаграммы валидны и с error-ветками |
| **Общая готовность к коду** | **🟢 ГОТОВО. P0/P1/P2 закрыты полностью.** |
