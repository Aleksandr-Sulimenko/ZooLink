...
- [ ] Завести репозиторий бэкенда, NestJS-скелет с модулями из §3.
- [ ] `schema.prisma` сгенерировать/сверить с `database_schema.sql`; типы Kysely.
- [ ] Docker Compose dev (api, postgres, redis, minio, worker) + reverse proxy.
- [ ] Адаптеры провайдеров (SmsProvider/EmailProvider/MapsProvider/ObjectStorage/PaymentProvider) — интерфейсы.
- [ ] OpenAPI-контракты сверить с `docs/03-architecture/api-contracts/*.yaml`.
- [ ] CI: lint + типы + миграции + dependency/SAST scan.
