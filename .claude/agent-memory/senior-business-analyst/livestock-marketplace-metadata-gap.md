---
name: livestock-marketplace-metadata-gap
description: Livestock marketplace documentation mentions JSONB metadata field not present in database schema for listings table
metadata:
  type: project
---

The livestock marketplace documentation (both English and Russian versions) states under Non-Functional Requirements -> Extensibility: "JSONB `metadata` field for experimental attributes (e.g., video URL, auction links, document uploads)." Additionally, the conceptual data model table for listings includes a `metadata` attribute of type JSONB.
